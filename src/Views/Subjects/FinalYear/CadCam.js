...
import { baseUrl, cadcam } from "../../../Constant/Api";
...
const CadCam = () => {
  ...
  const deleteHandler=(id)=>{
    ...
    if (warning) {
      axios.delete(`${baseUrl}/${cadcam}/${id}`).then(() => {
        setRender(!render);
      });
    }
  }
  ...
  const submitHandler = (e) => {
    e.preventDefault();
    axios.post(`${baseUrl}/${cadcam}`, inputquestion).then((res) => {
      if (res.data) {
  ...
  useEffect(() => {
    axios.get(`${baseUrl}/${cadcam}`).then((res) => {
