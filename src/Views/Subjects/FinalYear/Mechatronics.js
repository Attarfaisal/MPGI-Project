...
import { baseUrl, mechatronics } from "../../../Constant/Api";
...
const Mechatronics = () => {
  ...
  const deleteHandler=(id)=>{
    ...
    if (warning) {
      axios.delete(`${baseUrl}/${mechatronics}/${id}`).then(() => {
        setRender(!render);
      });
    }
  }
  ...
  const submitHandler = (e) => {
    e.preventDefault();
    axios.post(`${baseUrl}/${mechatronics}`, inputquestion).then((res) => {
      if (res.data) {
  ...
  useEffect(() => {
    axios.get(`${baseUrl}/${mechatronics}`).then((res) => {
